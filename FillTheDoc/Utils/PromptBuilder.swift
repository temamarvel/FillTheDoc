...
///
/// Это не просто helper со строками, а фактический контракт между приложением и LLM.
/// Именно здесь зафиксировано:
/// - какие поля модель обязана вернуть;
/// - какие нормализации допустимы, а какие запрещены;
/// - где разрешено безопасное «довыведение», а где нужно вернуть `null`;
/// - в каком JSON-формате приложение ждёт ответ.
///
...
enum PromptBuilder {
    ...
    static func user(sourceText: String) -> String {
        // User-prompt остаётся минималистичным: основная строгость и ограничения уже заданы
        // в system prompt. Здесь мы только даём модели сам текст и несколько эвристических подсказок
        // про то, где обычно лежат реквизиты.
        """
